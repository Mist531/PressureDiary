...
import io.pressurediary.server.api.models.*
import io.pressurediary.server.backend.authorization.AuthRouteUtils
import io.pressurediary.server.backend.configure.configure
import io.pressurediary.server.backend.database.tables.*
import io.pressurediary.server.backend.managersImpl.*
import io.pressurediary.server.backend.modules.commonModule
import io.pressurediary.server.backend.modules.managerModule
import io.pressurediary.server.backend.modules.managersImplModule
...
enum class BdConnect(val bdConnectInfo: BdConnectInfo) {
    ...
    Supabase(
        BdConnectInfo(
            ...
            bdPass = "pwd1095..sad",
            bdUser = "postgres.msvfyaghzjgbkbvakybc"
        )
    )
}
